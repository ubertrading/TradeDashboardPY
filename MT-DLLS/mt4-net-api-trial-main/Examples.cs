...
namespace RealTimeQuotes
{
    class Program
    {
        ...
        { 
            ...
            new Program().Run();
            //Program2.Main2(args);
            var ex = new ServerException(0);
            int code = (int)ex.Code;
        }
        ...
        {
            QuoteClient qc = new QuoteClient(990000185, "nwmm2wj", "150.60.249.152", 443);
            qc.Connect();
            var m_Symbol = qc.GetSymbolInfo("EURUSD").Ex;
            if (m_Symbol.trade == 0)
                Console.WriteLine("DISABLED");
            else if (m_Symbol.trade == 1)
                Console.WriteLine("CLOSE_ONLY");
            else if (m_Symbol.trade == 2)
                Console.WriteLine("FULL_ACCESS");
            else if (m_Symbol.trade == 3)
                Console.WriteLine("LONG_ONLY");
            else if (m_Symbol.trade == 4)
                Console.WriteLine("SHORT_ONLY");
        ...
        {
            QuoteClient qc = new QuoteClient(12006165, "DhXKp7oE", "170.75.202.214", 443);
            qc.Connect();
            qc.Subscribe("EURUSD");
            while (qc.ServerTime == default)
                Thread.Sleep(10);
            Console.WriteLine(qc.ServerTime + ". Press any key to quit...");
            Console.ReadKey();
        }

        private void TestSRV()
        {
            var main = QuoteClient.LoadSrv(@"ICMarkets-Demo02.srv");
            QuoteClient qc = new QuoteClient(20456309, "ipnu58", main.Host, main.Port);
            qc.Connect();
        ...
        void Reconnect()
        {
            ...
            Console.WriteLine("Connecting...");
            qc.OnDisconnect += delegate (object sender, DisconnectEventArgs args) { ((QuoteClient)sender).Connect(); };
            qc.Connect();
            Console.WriteLine("Connected to server");
        }
        ...
        {
            //QuoteClient qc = new QuoteClient(500414730, "jjm4cjy", "mt4-demo.roboforex.com", 443);
            //QuoteClient qc = new QuoteClient(24285192, "19870621525hF", "r11d00.exness.cn", 443); //r11d00.exness.cn 193.194.117.141 
            //QuoteClient qc = new QuoteClient(14867835, "x1hjhko", "23.111.102.16", 1119); // alpari demo
            //QuoteClient qc = new QuoteClient(321151, "abc123", "efumt5demo.itraderserver.com", 443);
            //QuoteClient qc = new QuoteClient(20065927, "bFWP68Hc", "54.150.153.168", 443);
            QuoteClient qc = new QuoteClient(500476959, "ehj4bod", "mt4-demo.roboforex.com", 443);
            //QuoteClient qc = new QuoteClient(8621, "6hzusrr", "47.75.127.65", 443);
            Console.WriteLine("Connecting...");
            qc.Connect();
            Console.WriteLine("Connected to server");
      ...
      while (qc.GetQuote(symbol).Ask == ask)
        ...
        void Ping()
        {
            ...
            foreach (var item in ar)
                Console.WriteLine(item.Description + " " + QuoteClient.PingHost(item.Host, item.Port));
        ...
        void MarketMany()
        {
            try
            {
                QuoteClient qc = new QuoteClient(2740468, "yliy8ks", "mt4-ng-practice.oanda.com", 443);
                string symbol = "EURUSD";
                OrderClient oc = new OrderClient(qc);
                Console.WriteLine("Connected to server");
                while (qc.GetQuote(symbol) == null)
        ...
        void RealtimeQuotes()
        {
            ...
            Console.WriteLine("Connecting...");
            qc.Connect();
            Console.WriteLine("Timezone  = " + qc.ServerTimeZone);
            qc.Subscribe("USDJPY");
            qc.OnQuote += delegate (object sender, QuoteEventArgs quote)
            {
                var qc = (QuoteClient)sender;
        ...
        {
            //QuoteClient qc = new QuoteClient(500414730, "jjm4cjy", "mt4-demo.roboforex.com", 443);
            //QuoteClient qc = new QuoteClient(360213, "dkwy7rt", "34.232.56.156", 443);
            //QuoteClient qc = new QuoteClient(500476959, "ehj4bod", "mt4-demo.roboforex.com", 443);
            QuoteClient qc = new QuoteClient(5139578, "jfRWhp@446", "185.96.244.18", 443);
            //QuoteClient qc = new QuoteClient(14990206, "hp1fxeb", "141.95.36.224", 1119);
            
                
            Console.WriteLine("Connecting...");
            qc.Connect();
            //qc.Subscribe(new string[] { "BTCUSD", "GBPUSD" });
            Console.WriteLine("Connected");
            string symbol = "U30USD";
            
            //var tv = qc.GetTickValue(symbol);
            Console.WriteLine("Tick Value " + qc.GetTickValue(symbol));
            //var inf = qc.GetSymbolInfo(symbol);
            Console.WriteLine("Tick Size " + qc.GetTickSize(symbol));
            //Console.WriteLine(qc.GetBidTickValue(qc.GetQuote(symbol, 10000))/inf.ContractSize);
            qc.OnQuote += Qc_OnQuote2;
            //qc.Subscribe(new string[] {"EURUSD", "GBPUSD"});
            Console.ReadKey();
           // qc.Unsubscribe(new string[] { "EURUSD", "GBPUSD" });
            Console.WriteLine("Press any key...");
            Console.ReadKey();
        ...
        {
            var qc = (QuoteClient)sender;
            Console.WriteLine(args + " " + qc.GetTickValue(args));
        }

        void AccountEquity()
        {
            //QuoteClient qc = new QuoteClient(500414730, "jjm4cjy", "mt4-demo.roboforex.com", 443);
            QuoteClient qc = new QuoteClient(6977656, "Qq123123", "47.91.190.248", 443);
            Console.WriteLine("Connecting...");
            qc.Connect();
            Console.WriteLine("Connected");
            qc.OnQuote += Qc_OnQuote;
            qc.Subscribe("EURUSDm");
            Console.WriteLine("Press any key...");
            Console.ReadKey();
        }
        ...
        private void Qc_OnQuote(object sender, QuoteEventArgs args)
        {
            var qc = (QuoteClient)sender;
            //Console.WriteLine(args);
            Console.WriteLine(qc.AccountEquity);
        }
        ...
        void Connect()
        {
            Logger.OnMsg += Logger_OnMsg;
            //QuoteClient qc = new QuoteClient(500414730, "jjm4cjy", "mt4-demo.roboforex.com", 443);
            //QuoteClient qc = new QuoteClient(89397762, "Y#E8yTE#", "d41a1.octanetwork.net", 444);
            //QuoteClient qc = new QuoteClient(89397762, "Y#E8yTE#", "d42a1.octanetwork.net", 443);
            QuoteClient qc = new QuoteClient(500476959, "ehj4bod", "mt4-demo.roboforex.com", 443);
            qc.Connect();
            foreach (var item in qc.Symbols)
                Sym.Add(item);
            qc.OnQuote += Qc_OnQuote1;
            qc.Subscribe(qc.Symbols);

            //qc.DataQC.OnQuote += Qc_OnQuote1;
            Console.WriteLine("Connected");
            qc.Subscribe("AAPL");
            //qc.DataQC.Subscribe("AAPL");
            Console.WriteLine("Press any key...");
            Console.ReadKey();
        }

        private void Qc_OnQuote1(object sender, QuoteEventArgs args)
        {
            Console.WriteLine(args);
            lock (Sym)
            {
                if (Sym.Contains(args.Symbol))
                    Sym.Remove(args.Symbol);
                if (Sym.Count == 0)
                {
                    ((QuoteClient)sender).Unsubscribe(((QuoteClient)sender).Symbols);
